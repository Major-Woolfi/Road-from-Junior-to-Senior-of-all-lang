...
Возвращает асинхронный итератор для асинхронного повторения. Эквивалентно вызову `x.__aiter__()`.
...
Возвращается `True` если все элементы повторения верны (или если повторение пусто). Эквивалентно:
...
Это вызывает `__anext__()` метод `async_iterator`, возвращение `awaitable`. В ожидании этого он возвращает следующее значение итераторa. Если `default` дан, он возвращается если итератор исчерпан, в противном случае `StopAsyncIteration` поднимается.
...
# ОСТАЛЬНОЙ ТЕКСТ В ПРОЦЕССЕ ФОРМАТИРОВАНИЯ И ИСПРАВЛЕНИЯ ОШИБОК

бин(Х)
Преобразовать целочисленное число в двоичную строку, префиксированную «0B». Результат является действительным выражением Python. Если Х не питон int объект, это должен определить **index**() Метод, который возвращает целое число. Некоторый примеры:
...
сорт буль(объект=ЛОЖЬ, /)
Вернуть логическую ценность, то есть один из True или False. Аргумент преобразуется с использованием стандарта Процедура тестирования истины. Если аргумент ложный или опущено, это возвращает False; В противном случае он возвращается True. А bool класс - это подкласс int (Видеть Числовые типы - int, float, комплекс). Это не может быть подкласс дальше. Его единственные случаи False и True (Видеть Логический тип - Bool).
